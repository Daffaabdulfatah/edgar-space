    ...
    async function loadCategory() {
      try {
        ...
        if (res.success && res.data) {
          ...
          setThumbnailUrl(cat.thumbnail || '');
        }
      } catch (err) {
        setError(err.message || 'Gagal memuat rincian kategori.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        res = await fetchApi(`/admin/categories/${categoryId}`, {
          method: 'PUT',
          body: formData
        });
      } else {
  ...
  if (loadingCategory) {
    return (
      <div className="py-20 flex flex-col items-center justify-center space-y-3">
        ...
        <p className="text-xs text-warm-gray">Memuat rincian kategori...</p>
      </div>
    );
  }
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs text-warm-gray font-light">
            Perbarui nama, deskripsi, dan thumbnail kategori produk.
          </p>
        </div>
      </div>
